...
function renderCharts(paid, remaining) {
    ...
    genderChart = new Chart(document.getElementById('genderChart'), {
        type: 'doughnut', data: { labels: ['ប្រុស', 'ស្រី'], datasets: [{ data: [maleCount, femaleCount], backgroundColor: ['#0d6efd', '#6f42c1'], hoverOffset: 4 }] },
...
function renderTable() {
    ...
    students.forEach((student, index) => {
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="btn btn-sm btn-info me-1" onclick="editStudent('${student.id}')" title="កែប្រែ"><i class="fas fa-edit"></i></button>
                <button class="btn btn-sm btn-danger" onclick="deleteStudent('${student.id}')" title="លុប"><i class="fas fa-trash-alt"></i></button>
            </td>
        `;
    });
...
window.editStudent = function (id) {
    ...
    if (student) {
        ...
        SAVE_BTN.innerHTML = '<i class="fas fa-save"></i> រក្សាទុកការកែប្រែ';
...
function resetForm() {
    ...
    SAVE_BTN.innerHTML = '<i class="fas fa-save"></i> រក្សាទុក';
}
